...
def display_step2_issue_diagnosis(analysis_data):
    ...
    # Display formatted_analysis or detailed_analysis if available
    # Check formatted_analysis first, then detailed_analysis
    analysis_text = None
    if 'formatted_analysis' in analysis_data and analysis_data['formatted_analysis']:
        analysis_text = analysis_data['formatted_analysis']
    elif 'detailed_analysis' in analysis_data and analysis_data['detailed_analysis']:
        analysis_text = analysis_data['detailed_analysis']
    ...
    if analysis_text:
        ...
        if isinstance(analysis_text, dict):
            ...
            analysis_text = str(analysis_text) if analysis_text is not None else ""
        
        # If the LLM included a prefixed "Formatted Analysis:" section, prefer its content
        try:
            import re
            formatted_block = re.search(r"Formatted Analysis:\s*(.*)$", analysis_text, re.DOTALL | re.IGNORECASE)
            if formatted_block and formatted_block.group(1).strip():
                analysis_text = formatted_block.group(1).strip()
        except Exception:
            pass
        ...
        try:
            ...
            analysis_text = _re_html.sub(r'<br\s*/?>', '\n\n', analysis_text, flags=_re_html.IGNORECASE)
            # Remove other common HTML tags that might leak through
            analysis_text = _re_html.sub(r'</?[a-z]+[^>]*>', '', analysis_text, flags=_re_html.IGNORECASE)
        except Exception:
            pass
        ...
        analysis_text = sanitize_persona_and_enforce_article(analysis_text)
        
        # Extract and render markdown tables first
        analysis_text = _extract_and_render_markdown_tables(analysis_text)
        
        # Normalize markdown formatting
        analysis_text = normalize_markdown_block_for_step3(analysis_text)
...
def display_forecast_graph_content(analysis_data, step_number=None, step_title=None):
    ...
    if step_number and step_title:
        ...
        header_title = f"📈 {step_prefix} {step_number} — {step_title}: {forecast_text}"
    else:
        forecast_text = t('pdf_5_year_yield_forecast_projections', '5-Year Yield Forecast & Projections')
    ...
    if forecast and isinstance(forecast, dict):
        ...
        try:
            ...
            if baseline_yield > 0:
                current_baseline_text = t('pdf_current_baseline', 'Current Baseline')
                fig.add_hline(
                    y=baseline_yield, 
                    line_dash="dash", 
                    line_color="gray",
                    annotation_text=f"{current_baseline_text}: {baseline_yield:.1f} t/ha",
            ...
            investment_scenarios = [
                ('high_investment', t('pdf_high_investment', 'High Investment'), '#e74c3c'),
                ('medium_investment', t('pdf_medium_investment', 'Medium Investment'), '#f39c12'),
                ('low_investment', t('pdf_low_investment', 'Low Investment'), '#27ae60')
            ]
            ...
            fig.update_layout(
                title=t('pdf_5_year_yield_projection_title', '5-Year Yield Projection from Current Baseline'),
                xaxis_title=t('pdf_years', 'Years'),
                yaxis_title=t('pdf_yield_tons_ha', 'Yield (tons/ha)'),
                xaxis=dict(
                    tickmode='array',
                    tickvals=years,
